...
- **Unrestricted File Upload / Stored XSS**: To prevent unrestricted file upload vulnerabilities, image and file uploads must be validated against a strict whitelist of allowed MIME types. Files falling into `FileCategory::Unknown` must be explicitly rejected (e.g., returning an `UnknownFileType` error) rather than implicitly handled with a fallback `application/octet-stream` MIME type, to avoid potential Stored XSS via malicious file types (like SVG).
